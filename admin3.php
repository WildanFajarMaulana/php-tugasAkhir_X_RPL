...
if ($_SESSION['level']=='user') {
  header("Location:index.php");
     ...
     $no=1;
     $datapts=query("SELECT * FROM tb_tambahinformasipts");
     if (isset($_POST["cari"])) {
     $datapts= caridatapts($_POST["keyword"]);
      }
        ...
        <div class="kotak3">
          ...
          <table class="table table-bordered" width="200px">
            <thead>
             ...
             <?php  foreach ($datapts as $pts ) : ?>
              <tr>
                <td><?php echo $no++ ?></td>
                <td><?php echo $pts['id']; ?></td>
                <td> <div class="dashboard-foto" style="background-image: url('img/<?php    echo $pts['fotouniversitas'];?>');"></div></td>
                <td ><?php echo $pts['namauniversitas']; ?></td>
                <td class="isi-univ"  style=""><?php echo $pts['isiuniversitas']; ?></td>
                <td><a href="deleteadminpts.php?id=<?php echo $pts['id'] ?>" class="btn btn-danger" onclick= "return confirm('anda yakin min?')">delete</a></td>
              </tr>
            <?php endforeach; ?>
            </table>
        </div>

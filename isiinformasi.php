...
$id_informasi=$_GET["id_informasi"];
 $query=querydataptn("SELECT * FROM tb_tambahinformasi where id_informasi='$id_informasi'");
...
$id_link=$_GET["id_informasi"];
 if (isset($_POST["submit"])) {
    if (tambahkomentarptn($_POST,$id,$id_link,$nama,$tanggal,$fotoprofil)>0) {
     header("Location:./isiinformasi.php?id_informasi=$id_link");
...
$datakomen=querydataptn("SELECT * FROM tb_kometarptn WHERE id_link='$id_link' ORDER BY id_komentar ASC");
 ?>
<!doctype html>
<html lang="en">
  <head>
                ...
                <?php   foreach ($datakomen as $komentar) : ?>
                    <div class="row">
                        <div class="col" >   
                            ...
                            <a href="hapuskomentarptn.php?id_komentar=<?php    echo $komentar['id_komentar']; ?>&id_link=<?php echo $komentar['id_link']; ?> "  onclick="return confirm('Anda Yakin Ingin Menghapusnya?');" ><i class="fas fa-trash"></i></a>
                        <?php   }else{
